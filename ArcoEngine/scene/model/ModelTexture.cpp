...
CModelTexture::CModelTexture(ID3D11Device *device, ID3D11DeviceContext *context)
	: CModelBase(device, context)
	, mCamera(nullptr), mMaterialTexture(nullptr)
{
}

CModelTexture::~CModelTexture(void)
{
	if (mMaterialTexture)
		delete mMaterialTexture;
}

void CModelTexture::Initialize(DXCamera *camera, const std::wstring &textname)
{
	mCamera = camera;
	mMaterialTexture = new MaterialTexture();
	mMaterialTexture->Initialize(
		"TextureShader", textname, posTexVertexLayoutDesc, posTexVertexLayoutSize,
		mpDevice, mpContext);
	CModelBase::Initialize(mMaterialTexture);
}

void CModelTexture::drawModel()
{
	setRenderProperties();
	activateBuffers();
...
void CModelTexture::setRenderProperties()
{
	ID3D11Buffer *conbuf;
